...
void FMODStudioModule::_bind_methods()
{
	...
	ClassDB::bind_method(D_METHOD("get_distance_scale_2d"), &FMODStudioModule::get_distance_scale_2d);
...
bool FMODStudioModule::initialize_fmod()
{
	...
	if (FMODSettings::get_singleton() != nullptr) {
		if(!FMODSettings::get_singleton()->already_initialized) {
...
{
	fmod_initialized = StaticCString::create("fmod_initialized");
...
//
// void FMODStudioModule::first_scene_tree_processed_frame() {
// 	print_line("scene tree has done first process");
// 	attempt_autoload_runtime();
// }
...
bool FMODStudioModule::init()
{
	...
	TypedArray<String> banks_to_load_at_startup = get_platform_project_setting(get_platform_setting_path(PlatformSettingsPath::FMOD_SETTINGS_PATH) + String("banks_to_load_at_startup"));
