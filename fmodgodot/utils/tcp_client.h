...
public:
	...
	Error get_command(const String& command, String& result)
	{
		...
		if (put_data((const uint8_t *)cs.get_data(), cs.length()) != Error::OK)
		{
			return Error::FAILED;
		}
		...
		if (bytes >= 0)
		{
			Array result_array = _get_data(bytes);
		...
		if (data_array.is_empty())
		{
			...
			{
				bytes_array = _get_data(get_available_bytes());
		...
		if (data_array[data_array.size() - 1] == 0)
		{
			String message;
			if (data_array.size() > 0) {
				const uint8_t *r = data_array.ptr();
				message.parse_utf8((const char *)r, data_array.size());
			}
