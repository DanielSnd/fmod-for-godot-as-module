...
static void debug_monitor(FMOD_DEBUG_FLAGS flags, const char* file, int line, const char* func, const char* message)
{
	...
	{
		print_line(String("[ERROR]: "), String(message));
	}
	else if (flags & FMOD_DEBUG_LEVEL_WARNING)
	{
		print_line(String("[WARNING]: "), String(message));
	}
	else if (flags & FMOD_DEBUG_LEVEL_LOG)
	{
		print_line(String("[LOG]: "), String(message));
	}
}
...
static String get_platform_setting_path(PlatformSettingsPath path)
{
	switch (path)
	{
		...
		case PlatformSettingsPath::GODOT_ANDROID_SETTING_POSTFIX:
			return ".android";
		case PlatformSettingsPath::FMOD_SETTINGS_ENABLED_PATH:
			return "fmod/settings/fmod_enabled";
		default:
			return "";
	}
}
...
#else
	if (setting == get_platform_setting_path(PlatformSettingsPath::FMOD_SETTINGS_ENABLED_PATH)) {
		if (project_settings && project_settings->has_setting(setting)) {
			bool fmod_enabled = static_cast<bool>(project_settings->get(setting));
			if(fmod_enabled) {
				ERR_PRINT("[FMOD] Is enabled however this platform isn't supported");
			}
		}
		return false;
	}
ERR_PRINT( "[FMOD] Platform not supported");
#endif
	//print_line("Get platform project setting ",setting," forcing agnostic? ",force_platform_agnostic);
...
static bool is_fmod_enabled() {
	return static_cast<bool>(get_platform_project_setting(get_platform_setting_path(PlatformSettingsPath::FMOD_SETTINGS_ENABLED_PATH)));
}
...
static inline void fmod_obj_to_path(const T& obj, String& out_path)
{
	...
	do
	{
		...
		{
			ERR_PRINT(vformat("[FMOD] Failed to get the Path of an FMOD Object %s %s %s", __FUNCTION__, __FILE__,
					__LINE__));
			break;
		}
