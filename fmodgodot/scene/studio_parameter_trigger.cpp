...
void StudioParameterTrigger::do_enter_tree()
{
	handle_game_event(GAMEEVENT_ENTER_TREE);
}

void StudioParameterTrigger::do_ready()
{
	if (Engine::get_singleton()->is_editor_hint())
	{
		if (is_inside_tree())
...
void StudioParameterTrigger::do_exit_tree()
{
	if (Engine::get_singleton()->is_editor_hint())
	{
		if (is_inside_tree())
...
void StudioParameterTrigger::handle_game_event(FmodGameEvent game_event)
{
	if (Engine::get_singleton()->is_editor_hint())
	{
		return;
...
void StudioParameterTrigger::set_trigger_on(FmodGameEvent _trigger_on)
{
	this->trigger_on = _trigger_on;
}

FmodGameEvent StudioParameterTrigger::get_trigger_on() const
{
	return trigger_on;
}

void StudioParameterTrigger::set_event(const Ref<EventAsset>& _event_asset)
{
	event = _event_asset;
}
