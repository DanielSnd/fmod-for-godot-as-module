...
void RuntimeUtils::to_3d_attributes_node(const Ref<FmodTypes::FMOD_3D_ATTRIBUTES> &attributes, Object* node,
                                         Object* physicsbody)
{
...
void RuntimeUtils::to_3d_attributes_transform_physicsbody(const Ref<FmodTypes::FMOD_3D_ATTRIBUTES> &attributes,
                                                          const Variant& transform, Object* physicsbody)
{
	...
	{
		WARN_PRINT(
				vformat("[FMOD] Converting to 3D Attributes is only available for Transform3D and Transfom2D. %s %s %s", __FUNCTION__,
				__FILE__, __LINE__));
	}
}

void RuntimeUtils::to_3d_attributes(const Ref<FmodTypes::FMOD_3D_ATTRIBUTES> &attributes, const Variant& position)
{
	FMOD_3D_ATTRIBUTES fmod_attributes{};
	float distance_scale_2d = FMODStudioModule::get_singleton()->distance_scale_2d;
	...
	switch (position.get_type())
	{
		...
		{
			WARN_PRINT(
					vformat("[FMOD] Converting to 3D Attributes is only available for Transform3D, Transform2D, Vector3 and Vector2. %s %s %s",
