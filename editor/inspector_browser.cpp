									...
									if (event_asset.is_valid())
									{
										...
										for (size_t i = 0; i < parameters.size(); i++)
										{
											...
											if (param_asset.is_valid())
											{
												...
												String path = param_asset->get_fmod_path();
...
bool FMODEditorInspectorTree::update_filter(TreeItem* p_parent, bool p_scroll_to_selected)
{
	...
	TreeItem* _root = root_item;
	TypedArray<TreeItem> children = _root->get_children();
...
void FMODEditorInspectorTree::on_item_collapsed(Object* item)
{
	...
	if (tree_item->has_meta("Type"))
	{
		String _type = tree_item->get_meta("Type");

		if (_type == String("folder"))
		{
...
void FMODEditorInspectorProperty::popup_menu(PopupType _type, Vector2 pos)
{
	switch (_type)
	{
...
void FMODEditorInspectorProperty::gui_input(const Ref<InputEvent>& event)
{
...
void FMODEditorInspectorProperty::on_item_selected()
{
	...
	if (selected_item)
	{
		...
		String _type = selected_item->get_meta("Type");
...
void FMODEditorInspectorProperty::on_event_popup_id_pressed(int32_t id)
{
	...
	switch (id)
	{
		...
		{
			String path = event->get_fmod_path();
...
void FMODEditorInspectorProperty::set_editor_scale(float _editor_scale)
{
	this->editor_scale = _editor_scale;
}
